...
Strategy.prototype._verify = function(req, data, done){
  ...
  fetches.push($userQuery);

  if (req && req.query && req.query.profkey) {
    $term = model.query('terms', {profKey: req.query.profkey});
    fetches.push($term);
  }
  ...
  model.fetch(fetches, function(err) {
    ...
    profId = profId || req.query.profIf;
    termId = termId || req.query.termId;
    ...
    if (user) {
      ...
      if (user[self.name]){
        ...
        if (!user.prof) {
          if (profId){
            upgrade.profId = profId;
            upgrade.profIdAccepted = true;
          }

          upgrade.termId = termId;
        }
